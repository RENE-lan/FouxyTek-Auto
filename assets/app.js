...
const NAV=[
  ...
  ["realisations.html","Réalisations"],["faq.html","FAQ"],["apropos.html","À propos"]
];
...
function chrome(){
  ...
  waFab.href=`https://wa.me/${WA}?text=${encodeURIComponent("Bonjour FouxyTek 👋 Je voudrais avoir des informations sur vos écrans CarPlay.")}`;
  waFab.target='_blank';waFab.rel='noopener';waFab.setAttribute('aria-label','Contacter sur WhatsApp');
  waFab.innerHTML=IC.wa;
...
function initConfig(){
  ...
  function reco(){
    ...
    const veh=[b.value,m.value,y.value].filter(v=>v&&!/^Autre|^Je ne sais/.test(v)).join(' ')||"votre véhicule";
    ...
    const msg=soft?`Bonjour FouxyTek 👋 J'ai ${veh} et je ne suis pas sûr du modèle exact. Pouvez-vous confirmer l'écran idéal (je peux envoyer une photo du tableau de bord) ?`
                  :`Bonjour FouxyTek 👋 Je possède un(e) ${veh} et je voudrais le prix du ${name} avec installation.`;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  syncList();
  initProducts();initConfig();initBooking();initChat();initFAQ();initGallery();initPolish();initFx();
});
